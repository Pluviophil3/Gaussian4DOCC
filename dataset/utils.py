...
def custom_collate_fn_temporal(instances):
    return_input_dict, return_prev_dict = {},{}
    input_dict, prev_dict = instances[0]
    
    instances  = [input_dict]
    for k, v in instances[0].items():
        if isinstance(v, np.ndarray):
            return_input_dict[k] = torch.stack([
                torch.from_numpy(instance[k]) for instance in instances])
        elif isinstance(v, torch.Tensor):
            return_input_dict[k] = torch.stack([instance[k] for instance in instances])
        elif isinstance(v, (dict, str)):
            return_input_dict[k] = [instance[k] for instance in instances]
        elif isinstance(v, list):
            return_input_dict[k] = [instance[k] for instance in instances]
        elif v is None:
            return_input_dict[k] = [None] * len(instances)
        else:
    ...
    for k, v in instances[0].items():
        if isinstance(v, np.ndarray):
            return_prev_dict[k] = torch.stack([
                torch.from_numpy(instance[k]) for instance in instances])
        elif isinstance(v, torch.Tensor):
            return_prev_dict[k] = torch.stack([instance[k] for instance in instances])
        elif isinstance(v, (dict, str)):
            return_prev_dict[k] = [instance[k] for instance in instances]
        # elif isinstance(v, list):
        #     return_dict[k] = v
        elif v is None:
            return_prev_dict[k] = [None] * len(instances)
        else:
            raise NotImplementedError
    return (return_input_dict ,return_prev_dict)
