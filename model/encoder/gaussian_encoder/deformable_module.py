...
class SparseGaussian3DKeyPointsGenerator(BaseModule):
    ...
    def forward(
        ...
        key_points = key_points + xyz.unsqueeze(2)# B N P 3
        pc_range = torch.tensor(self.pc_range, device=key_points.device)  # (6,)
        pc_min = pc_range[:3]  # (3,)
        pc_max = pc_range[3:]  # (3,)
        temp_key_points = []
        last_pos = torch.tensor(metas[-1]['can_bus'][0][:3], 
                              device=key_points.device, 
                              dtype=torch.float32)
        for meta in metas:
            curr_pos = torch.tensor(meta['can_bus'][0][:3], 
                                  device=key_points.device, 
                                  dtype=torch.float32)
            translation = (last_pos - curr_pos) / (pc_max - pc_min)
            temp_key_points.append(key_points - translation[None, None, None, :])
        return temp_key_points # temproal_cnt B N P 3
...
class DeformableFeatureAggregation(BaseModule):
    def __init__(
        ...
        self.merge = nn.Linear(embed_dims * num_frames, embed_dims)
        if use_camera_embed:
    ...
    def forward(
        ...
        feature_maps, # cnt B N C H W
        metas: dict, # cnt ...
        anchor_encoder=None,
        **kwargs: dict,
    ):
        bs, num_anchor = instance_feature.shape[:2]
        key_points_list = self.kps_generator(anchor, instance_feature, metas)
        if self.use_deformable_func:
            processed_features = []
            for temp_feature_maps in feature_maps:
                processed_features.append(
                    DAF.feature_maps_format(temp_feature_maps)
                )
        ...
        for (
            ...
            weights = self._get_weights(
                instance_feature, anchor_embed, temp_metas
            )
            if self.use_deformable_func:
                weights = (
            ...
            temp_features_next = temp_features_next.sum(dim=2)
            features.append(temp_features_next)
