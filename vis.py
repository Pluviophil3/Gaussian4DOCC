...
def save_occ(
    ...
    scene.render()
    filepath = os.path.join(save_dir, f'{name}_bev.png')
...
def save_gaussian(save_dir, gaussian, gt_occ, name, ignore_opa=True, filter_zsize=False, scalar = 2):
    empty_label = 17
    ...
    if ignore_opa:
        top_indices = np.argsort(opas)[-25600:]
        mask = (pred != empty_label) & np.isin(np.arange(len(opas)), top_indices)
    else:
        mask = (pred != empty_label) & (opas > 0.7)
